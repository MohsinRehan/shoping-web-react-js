  ...
  const TopHeader = () => {
    const temp=JSON.parse(localStorage.getItem("acces_data"))
    const userImg= temp && temp.picture;
    ...
    return (
      <header className="h-24 w-full flex items-center  relative  px-5 space-x-10 bg-black">
        <div className="flex items-center mt-4 w-[20%]">
          ...
          <div className="ml-4">
            <h1 className="text-md font-medium text-white">Welcome Back</h1>
            <h1 className="text-sm font-regular text-white">{temp && temp.name}</h1>
        ...
        <div className="flex justify-between w-[80%]">
          ...
          <div className="flex flex-shrink-0 items-center space-x-4 text-white relative">
            ...
            <div  className="before:content-[''] before:w-[1px] before:h-10 before:bg-white before:absolute before:left-[104px]"><img src={userImg} className="h-10 w-10 rounded-full cursor-pointer border-2 border-blue-400" /></div>
          </div>
        </div>
      </header>
    );
